...
type PromptState = "hidden" | "needs-permission" | "blocked" | "needs-test";
...
export default function PushNotificationPrompt() {
  ...
  const refreshStatus = useCallback(async () => {
    ...
    if (Notification.permission === "granted") {
      const nextTestedKey = `bilad_push_tested_v2_${status.user_id}`;
      setPromptState(
        status.push_subscription_count > 0 &&
          localStorage.getItem(nextTestedKey) === "1"
          ? "hidden"
          : "needs-test"
      );
      return status;
  ...
  async function handleEnable() {
    ...
    try {
      ...
      if (Notification.permission !== "granted") {
        setMessage("Bildirim izni verilmedi. Telefon ayarlarindan izin acilmali.");
        return;
      }

      if (!status || status.push_subscription_count < 1) {
        setMessage("Abonelik kaydedilemedi. Sayfayi yenileyip tekrar deneyin.");
        return;
      }

      const nextTestedKey = `bilad_push_tested_v2_${status.user_id}`;
      setPromptState("needs-test");
      ...
      if (!response.ok) {
        setMessage("Test bildirimi gonderilemedi.");
        return;
      }
      ...
      if (sent > 0) {
        localStorage.setItem(nextTestedKey, "1");
        setMessage("Test bildirimi gonderildi. Telefonun ust bildirim alanini kontrol edin.");
        return;
      }
      ...
      setMessage(
        failed > 0
          ...
          : "Abonelik var ama aktif cihaz bulunamadi. Sayfayi yenileyip tekrar deneyin."
      );
    } catch {
      setMessage("Telefon bildirimi hazirlanirken hata olustu.");
    } finally {
      setLoading(false);
    }
  }
  ...
  const needsOnlyTest = promptState === "needs-test";
  ...
  return (
    <div className="fixed inset-x-4 bottom-4 z-50 mx-auto max-w-md rounded-3xl border border-sky-100 bg-white p-4 shadow-2xl shadow-slate-200/70">
      <div className="flex items-start gap-3">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-sky-50 text-lg">
          !
        </div>
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-xs leading-5 text-slate-500">
            {isBlocked
              ? "Ust bildirimler telefon veya Chrome ayarlarinda kapali gorunuyor. Izin acilinca izin talepleri ust bildirim olarak da gelir."
              : needsOnlyTest
                ? "Bu cihaz bildirim icin kayitli gorunuyor. Ust bildirimi dogrulamak icin bir test gonderin."
                : "Ust bildirimlerin gelmesi icin bu cihazda bir kez izin verip test bildirimi gonderin."}
          </p>
